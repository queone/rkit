...
`attune` is a small declarative reconciler: describe provider-neutral intent in
YAML and compare it directly with live Azure Resource Manager and Microsoft
Graph state. It supports DNS record sets, security groups, app registrations,
role definitions, role assignments, and resource groups. DNS values are
compared without ordering sensitivity, resource-group tags use merge semantics,
and the live provider remains the source of truth—there is no state backend,
initialization step, or local state file.
...
`validate` parses and checks specifications without credentials or provider
access. `plan` authenticates and reads live state but does not mutate it.
`apply` performs the reviewed create and update actions plus only those deletes
enabled by the applicable prune policy. Every live run prints non-secret
provider grounding so the operator can confirm its target. HTTPS connections
verify peers through the shared rkit trust loader, including OpenSSL certificate
paths and macOS Keychain roots.
...
`rncap` asks for confirmation before renaming every current-directory entry, including directories and symlinks. It title-cases Unicode letter/digit words across punctuation. Existing destinations are reported on standard error and skipped, while other rename errors are reported and processing continues.
...
`rnlower` asks for confirmation before renaming every current-directory entry, including directories and symlinks. It applies Unicode lowercase conversion. Existing destinations are reported on standard error and skipped, while other rename errors are reported and processing continues.

### sms

```text
sms v2.0.1
SMS CLI utility 2.0.1
...
`vdrop` removes a section and joins the remaining parts. Timestamps accept `MM:SS`, whole seconds, and `HH:MM:SS` for sources longer than one hour; `END` may be omitted or set to `end`. Outputs are named beside the input with `_` inserted before trailing digits, and existing outputs are never overwritten. The default path stream-copies at keyframes; `-a, --accurate` re-encodes for frame accuracy. `-x` or `--crossfade[=SECS]` dissolves an interior join, defaulting to 0.5 seconds. It requires `ffmpeg` and `ffprobe` on `PATH` and prints an input/output summary after success.
...
`vkeep` extracts a section. Timestamps accept `MM:SS`, whole seconds, and `HH:MM:SS` for sources longer than one hour; `END` may be omitted or set to `end`. Outputs are named beside the input with `_` inserted before trailing digits, and existing outputs are never overwritten. The default path stream-copies at keyframes; `-a, --accurate` re-encodes for frame accuracy. It requires `ffmpeg` and `ffprobe` on `PATH` and prints an input/output summary after success.
