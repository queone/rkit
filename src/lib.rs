...
where
    ...
    S: Into<OsString>,
{
    run_with(args, &Filesystem, ColorMode::detect())
}
...
mod tests {
    ...
    fn renders_sorted_nested_tree_with_dotfiles_and_symlink_as_file() {
        ...
        let result = run_with(std::iter::empty::<&str>(), &filesystem, ColorMode(false)).unwrap();
        assert_eq!(
            result.stdout(),
            "├── .hidden\n├── alpha.txt\n├── link\n├── nested\n\
             │   ├── wide界.txt\n│   └── z.txt\n└── βeta.txt\n"
    ...
    fn aligns_joined_paths_by_unicode_scalar_count() {
        ...
        let result = run_with(["-f"], &filesystem, ColorMode(false)).unwrap();
        assert_eq!(
            result.stdout(),
            "├── a.txt    a.txt\n└── 界.txt    界.txt\n"
        );
    ...
    fn renders_exact_colors_when_forced() {
        ...
        let result = run_with(["-f"], &filesystem, ColorMode(true)).unwrap();
        ...
        let help = run_with(["--help"], &filesystem, ColorMode(true)).unwrap();
    ...
    fn derives_version_from_cargo_metadata() {
        let filesystem = source([]);
        let result = run_with(["--version"], &filesystem, ColorMode(false)).unwrap();
        assert_eq!(
            result.stdout(),
            format!("tree v{}\n", env!("CARGO_PKG_VERSION"))
        );
    ...
    fn non_utf8_name_returns_runtime_error_without_output() {
        ...
        let error =
            run_with(std::iter::empty::<&str>(), &filesystem, ColorMode(false)).unwrap_err();
    ...
    fn non_utf8_root_returns_runtime_error_without_output() {
        ...
        let error = run_with([root], &source([]), ColorMode(false)).unwrap_err();
        assert_eq!(error.exit_code(), 1);
